...
export default async function handler(
    ...
    if (!session || !state || decodeURIComponent(session) !== decodeURIComponent(state)) return res.status(401).json({ error: "You do not have a valid session! Please return to /discord/auth" });

    const code = req.query.code;

    if (!code || typeof code !== 'string') return res.status(400).json({ error: "No code query!" })
	...
	const response = await axios.post("https://discord.com/api/v9/oauth2/token", formBody, {
        headers: {
