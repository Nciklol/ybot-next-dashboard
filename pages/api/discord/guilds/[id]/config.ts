...
export default async function handler(
    ...
    if (!doc.guilds?.some((g: Guild) => g.id === guildID)) return res.status(403).json({ error: "Not authorized to get this guild!"})
    ...
    return res.status(200).json({ "success": "Everything went well! Thanks for making me send this, nextapi."});
}